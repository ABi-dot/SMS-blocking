    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((ListView) findViewById(android.R.id.list)).setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                ...
                new AlertDialog.Builder(AddActivityView.this).setTitle("提醒")
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (flag == 1)
                            dbWrite.delete("blockWords", "_id=?", new String[]{itemId + ""});
                        else if (flag == 2) {
                            dbWrite.delete("blockNumber", "_id=?", new String[]{itemId + ""});
                        }
                        refreshListView();
                    }
        ...
        addKeywordsListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                String keywords = "";
                if (flag == 1) {
                    keywords = add_edit_keywords.getText().toString().trim();//提取对话框中的文字
                } else if (flag == 2) {
                    keywords = add_edit_address.getText().toString().trim();
                }

                if (!keywords.equals("")) {//内容不能为空
                    if (flag == 1) {
                        cv.put("keywords", keywords);
                        dbWrite.insert("blockWords", null, cv);//插入数据
                    } else if (flag == 2) {
